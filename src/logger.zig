...
// We have to use u8 here, otherwise we redeclare it and cast u8 to char which results in
...
fn logWithPrefix(comptime level: LogLevel, comptime fmt: []const u8, args: anytype) void {
    ...
    const msg = std.fmt.bufPrint(buffer[0 .. buffer.len - 1], prefix_fmt, args) catch {
        _ = printf(@as([*c]const u8, @ptrCast("[ERROR] logger formatting failed\n")));
        return;
    };
    ...
    _ = printf(@as([*c]const u8, @ptrCast(msg.ptr)));
}
