...
pub const Environment = struct {
    ...
    pub fn writeEnvironment(self: *Self, pid: i32) u8 {
        ...
        // Serialize and write
        const json = try std.json.Stringify.valueAlloc(self.allocator, self.data, .{ .whitespace = .indent_2 });
...
test "json serialization" {
    ...
    const json = try std.json.Stringify.valueAlloc(std.testing.allocator, env.data, .{ .whitespace = .indent_2 });
...
test "linked libraries serialization" {
    ...
    try env.libs.libraries.append(alloc, .{
    ...
    const json = try std.json.Stringify.valueAlloc(alloc, env.data, .{ .whitespace = .indent_2 });
