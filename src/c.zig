...
pub const InstrumentHooks = instrument_hooks.InstrumentHooks;
...
pub fn instrument_hooks_set_executed_benchmark(hooks: ?*InstrumentHooks, pid: i32, uri: []const u8) u8 {
    if (hooks) |h| {
...
pub fn instrument_hooks_executed_benchmark(hooks: ?*InstrumentHooks, pid: i32, uri: []const u8) u8 {
...
pub fn instrument_hooks_add_marker(hooks: ?*InstrumentHooks, pid: i32, marker_type: u8, timestamp: u64) u8 {
    if (hooks) |h| {
...
pub fn instrument_hooks_set_environment_list(
    ...
    if (hooks) |h| {
        ...
        const u8_values: []const []const u8 = @ptrCast(values);
        for (0..count) |i| {
...
pub fn instrument_hooks_callgrind_add_obj_skip(io: std.Io, path: ?[]const u8) u8 {
    ...
    _ = std.Io.Dir.realPathFileAbsolute(io, path_slice, real_path[0..std.fs.max_path_bytes]) catch return 0;

    valgrind_helper.callgrind_add_obj_skip(@ptrCast(&real_path[0]));
    return 0;
}

pub fn instrument_hooks_write_environment(hooks: ?*InstrumentHooks, pid: i32) u8 {
    if (hooks) |h| {
...
test "callgrind_add_obj_skip is a no-op outside valgrind" {
    ...
    try std.testing.expectEqual(@as(u8, 1), instrument_hooks_callgrind_add_obj_skip(std.testing.io, null));
    try std.testing.expectEqual(@as(u8, 0), instrument_hooks_callgrind_add_obj_skip(std.testing.io, @ptrCast("/nonexistent/path/that/will/not/resolve")));
    try std.testing.expectEqual(@as(u8, 0), instrument_hooks_callgrind_add_obj_skip(std.testing.io, @ptrCast("/")));
}
