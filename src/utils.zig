...
//
// This wrapper converts Zig string literals to null-terminated u8 arrays and handles
// variadic argument forwarding to the C printf function.
pub fn print(comptime fmt: []const u8, args: anytype) void {
    // Create a comptime null-terminated u8 array from the format string
    const fmt_z = comptime blk: {
        var buf: [fmt.len:0]u8 = undefined;
        for (fmt, 0..) |byte, i| {
            buf[i] = byte;
        }
        buf[fmt.len] = 0;
