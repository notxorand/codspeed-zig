...
pub const RunnerFifo = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, io: std.Io) !Self {
        return .{
            .allocator = allocator,
            .io = io,
            .writer = try fifo.Pipe.openWrite(allocator, io, shared.RUNNER_CTL_FIFO),
            .reader = try fifo.Pipe.openRead(allocator, io, shared.RUNNER_ACK_FIFO),
        };
    }
    ...
    pub fn set_executed_benchmark(self: *Self, pid: i32, uri: []const u8) !void {
...
test "test runner fifo" {
    ...
    try fifo.Pipe.create(io, shared.RUNNER_ACK_FIFO);
    try fifo.Pipe.create(io, shared.RUNNER_CTL_FIFO);

    var ctl_fifo = try fifo.Pipe.openRead(allocator, io, shared.RUNNER_CTL_FIFO);
    defer ctl_fifo.deinit();

    var ack_fifo = try fifo.Pipe.openWrite(allocator, io, shared.RUNNER_ACK_FIFO);
    ...
    var runner_fifo = try RunnerFifo.init(allocator, io);
